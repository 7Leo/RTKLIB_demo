...
//计算设计矩阵与伪距残差矩阵//H就是输出的设计矩阵，v就是输出的残差阵，var则是观测值的权重
...
{
    gtime_t time;
    double r,freq,rr[3],pos[3],dtr,e[3],P;
...
static int estpos(const obsd_t *obs, int n, const double *rs, const double *dts,
    ...
    //obs/nav/opt分别为观测值/星历/配置，而且都是不可修改的const变量
    //rs/dts/vare/svh分别为卫星位置/卫星钟差/观测值方差/卫星健康状态
    //sol为解算结果，azel为方位角/高度角，vsat为有效卫星数，resp为伪距残差,msg为错误信息
{
    ...
    //迭代收敛，直接设置循环次数不太合理，可以根据状态量是否收敛来终止循环
    for (i=0;i<MAXITR;i++) {
