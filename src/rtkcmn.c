...
* args   : double *A        I   transpose of (weighted) design matrix (n x m) 设计矩阵
*          double *y        I   (weighted) measurements (m x 1) 残差/观测值残差
*          int    n,m       I   number of parameters and measurements (n<=m)
*          double *x        O   estmated parameters (n x 1) 最小二乘估计得到的状态量(相对概略位置的修正量)
...
* transform ecef vector to local tangental coordinate ECEF转为局部切坐标
* args   : double *pos      I   geodetic position {lat,lon} (rad)
...
*          double *e        O   line-of-sight vector (ecef)   输出的e即为各个卫星的设计矩阵
